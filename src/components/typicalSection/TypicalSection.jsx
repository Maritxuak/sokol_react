...
const TypicalSection = ({apiGet}) => {
    ...
    const handleDetailsClick = async () => {
        if (selectedSlideId) {
            try {
                const response = await apiGet.get(`/read/project/${id}/similar/`);
    ...
    return (
        <section className="project-services-section">
            <div className="container container--size-md project-services-section__container">
                <h2 className="project-services-section__title">Похожие проекты</h2>
                        ...
                        {dignities.map((item) => (
                            <div key={item.id} className="swiper-slide project-services-section__slider-slide" data-our-workflow-slide-project={item.id}>
                                ...
                                <div className="project-services-section__slider-content">
                                    ...
                                    <a href={`/project/${item.id}`} key={item.id} to={`/project/${item.id}`} className="btn btn--size-md btn--theme-accent-fill project-services-section__slider-btn" onClick={handleDetailsClick}>
